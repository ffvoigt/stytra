...
# gain that converts the tail displacement to grating movements is modified
...
class GivingUpProtocol(Protocol):
    name = "givingup"
    stytra_config = dict(
        display=dict(min_framerate=30),
        tracking=dict(embedded=True, method="tail", estimator="vigor"),
        camera=dict(type="ximea", roi=[192, 30, 640, 614])
     )
    #stytra_config = dict(
    ...
    def __init__(self):
        ...
        self.mult = Param(1)
        self.low_gain = Param(0)
        self.mid_gain = Param(0.75)
    ...
    def get_stim_sequence(self):
        ...
        #  it n_reps times
        gain_values = (
            [self.high_gain*self.mult] * 1
            + [self.mid_gain*self.mult] * 1
            + [self.low_gain*self.mult] * 1
        ) * n_reps
...
if __name__ == "__main__":
    Stytra(protocol=GivingUpProtocol())
